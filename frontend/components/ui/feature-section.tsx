    ...
    <div className={cn("p-4 md:p-12", className)}>
      <div className="max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col md:grid md:grid-cols-2 gap-6 md:gap-10">
          ...
          <div className="order-2 md:order-1 mt-6 md:-mt-32">
            <div className="relative h-[180px] md:h-[600px] overflow-hidden">
              {visibleItems.map((feature, index) => {
                const displayIndex = (currentFeature + index) % features.length
                return (
                  <motion.div
                    key={displayIndex}
                    className={cn("absolute w-full", index !== 0 && "hidden md:block")}
                    animate={{ y: index * 200, opacity: 1 }}
                    transition={{ duration: 0.6, ease: "easeInOut" }}
                  >
                    <motion.div
                      className={cn(
                        "flex items-center gap-4 md:gap-8 p-4 md:p-6 rounded-lg transition-all duration-300 mb-4",
                        index === 0
                          ? "bg-blue-50 border-2 border-blue-500 scale-100 shadow-sm"
                          : "bg-gray-50 border border-gray-200 scale-95 opacity-75"
                      )}
                    >
                      <motion.div
                        className={cn(
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs md:text-base text-gray-600 line-clamp-3 md:line-clamp-2">
                          {feature.content}
                        </p>
                      </div>
                    </motion.div>
