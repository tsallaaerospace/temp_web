...
const useIntersectionObserver = (ref: React.RefObject<HTMLDivElement | null>) => {
...
const StatItem = ({ value, label, hasPlus = true }: { value: number; label: string; hasPlus?: boolean }) => {
  ...
  return (
    <div ref={ref} className="text-center">
      <h2 className="text-5xl md:text-6xl lg:text-7xl font-orbit font-bold text-white">
        {count}
        {hasPlus && '+'}
      </h2>
      <p className="text-base font-orbit md:text-lg text-gray-400 mt-2">{label}</p>
    </div>
  );
};
